...
esp_err_t game_get_handler(httpd_req_t *req){
    ...
    if(file==NULL){
        ESP_LOGE(TAG,"no se pudo abrir el archivo game.html");
        httpd_resp_send_404(req);
        return ESP_FAIL;
    }
...
esp_err_t start_player(httpd_req_t *req){
    ...
    if(player1 && player2){
        //se brinca la parte de player de abmos para ahora si estar en el nombre como 
        //tal que ingreso el usuario, y sabemos que el final de esa cadena esta en terminacion 
        //de caracter nulo
        player1+=8;
        player2+=8; 
