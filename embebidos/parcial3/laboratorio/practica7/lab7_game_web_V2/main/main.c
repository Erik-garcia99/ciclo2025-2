...
EventGroupHandle_t event_game, gpio_finish;
...
void app_main(void){
    ...
    gpio_finish = xEventGroupCreate();
    event_game= xEventGroupCreate();

    Mutex = xSemaphoreCreateMutex();

    //el grupo de eventos no regresa nada, por lo que debemos de checar la varibale 

    if(gpio_finish == NULL){
        ESP_LOGI(TAG, "el grupo de eventos no se creo correctamante!");
    }
    ...
    while (1) {
        if (xQueueReceive(button_queue, &pin_number, portMAX_DELAY)) {
            if (pin_number == PLAYER_1) {
                ESP_LOGI(TAG, "Jugador 1 finalizó el juego");
                xEventGroupSetBits(event_game, py1_btn_bit);
            } else {
                ESP_LOGI(TAG, "Jugador 2 finalizó el juego");
...
void display_task(void *params) {
    while (1) {
        ...
        // En una implementación real, aquí podrías actualizar una pantalla conectada al ESP32
...
void init_game(void) {
    ...
    // strcpy(game_state.player1_name, "Jugador 1");
    // strcpy(game_state.player2_name, "Jugador 2");
